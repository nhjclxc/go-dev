...
func init() {
	...
	// 1、初始化一个任务
	task1 := task.NewTask("task1", "0/5 * * * * *", func(ctx context.Context) error {
		fmt.Println("定时任务执行时间:", time.Now().Format("2006-01-02 15:04:05"))
		return nil
	})
...
// 定时任务
// https://beegodoc.com/zh/developing/task
func main() {

	task.StartTask() // 启动调度器
	defer task.StopTask() // 程序退出时关闭任务调度器
