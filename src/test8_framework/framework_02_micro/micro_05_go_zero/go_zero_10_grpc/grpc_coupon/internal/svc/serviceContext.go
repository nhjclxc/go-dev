...
import (
	...
	"grpc_coupon/grpc/grpcorderservice"
	"grpc_coupon/grpc/grpcuserservice"
)

type ServiceContext struct {
	Config config.Config
	GrpcOrderService grpcorderservice.GrpcOrderService  // 手动代码, 建立rpc连接
	GrpcUserService grpcuserservice.GrpcUserService  // 手动代码, 建立rpc连接
}

func NewServiceContext(c config.Config) *ServiceContext {

	grpcOrderClient, err := zrpc.NewClient(c.RpcOrderConf)
	if err != nil {
		//return nil
	}
	...
	return &ServiceContext{
		...
		GrpcOrderService: grpcorderservice.NewGrpcOrderService(grpcOrderClient), // 手动代码, 建立rpc连接
