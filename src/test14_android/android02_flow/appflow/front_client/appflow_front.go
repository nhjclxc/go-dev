...
func main() {
	...
	if *pkgList == "" {
		fmt.Println("Usage: ./appflow_front --pkgs=com.feedying.live.mix,cn.miguvideo.migutv [--server=http://192.168.201.167:8080]")
		return
	}
	...
	for {
		for i := range apps {
			...
			if err != nil {
				fmt.Println("GetForegroundApp检测失败:", err)
				continue
			}
			fmt.Println("当前前台应用包名:", frontPkg)

			if frontPkg != app.Package {
				continue
			}
...
GOOS=android GOARCH=arm CGO_ENABLED=1 go build -o appflow_front appflow_front.go
adb push appflow_front /data/local/tmp/
adb shell
cd /data/local/tmp
chmod +x appflow_front
./appflow_front --pkgs=cn.miguvideo.migutv,com.feedying.live.mix \
         --server=http://192.168.201.167:8080 \
         --interval=3 --report-cycle=30

./appflow_front --interval=3 --report-cycle=15 --pkgs=cn.miguvideo.migutv,com.feedying.live.mix,cn.juqing.cesuwang_tv --server=http://192.168.201.167:8080
./appflow_front --interval=3 --report-cycle=15 --pkgs=cn.juqing.cesuwang_tv --server=http://192.168.201.167:8080
./appflow_front --interval=15 --report-cycle=15 --pkgs=cn.juqing.cesuwang_tv --server=http://192.168.201.167:8080
