...
import (
	...
	"base_project/config"
	"base_project/pkg/clickhouse"
	"base_project/pkg/database"
	"base_project/pkg/logger"
	pkgredis "base_project/pkg/redis"
	"base_project/pkg/telemetry"
)
...
// WithTelemetry 初始化遥测（Tracing + Metrics）
func WithTelemetry() Option {
	return func(a *App) error {
		tel, err := telemetry.New(&a.Config.Telemetry)
		if err != nil {
			return fmt.Errorf("初始化遥测失败: %w", err)
		}
		a.Telemetry = tel
		return nil
	}
...
func (a *App) Close() error {
	...
	// 关闭 Telemetry（先关闭，确保所有 span 都被导出）
	if a.Telemetry != nil {
		if err := a.Telemetry.Shutdown(context.Background()); err != nil {
			errs = append(errs, fmt.Errorf("关闭遥测失败: %w", err))
		}
	}
