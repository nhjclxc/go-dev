...
// NewPriorityQueue 创建 PriorityQueue
func NewPriorityQueue(ctx context.Context, concurrentNum int64) *PriorityQueue {
...
func (b *PriorityQueue) Name() string {
	return "PriorityQueue"
}

// Enqueue 投递任务（生产）
func (b *PriorityQueue) Enqueue(ctx context.Context, queueTask *queue.QueueTask) error {
...
func (b *PriorityQueue) Dequeue(ctx context.Context) (*queue.QueueTask, error) {
	select {
	case <-b.ctx.Done():
		fmt.Println("context canceled. [任务队列被关闭]")
	...
	case queueTask := <-b.taskCh:
		log.Println("<获取任务队列数据成功>", queueTask)
		return queueTask, nil
	}
}

func (b *PriorityQueue) Ack(ctx context.Context, queueTask *queue.QueueTask) error {
...
// Nack 失败处理（重试 / 丢弃 / 延迟）
func (b *PriorityQueue) Nack(ctx context.Context, queueTask *queue.QueueTask, delayDuration time.Duration, reason error) error {
	select {
	case <-b.ctx.Done():
		fmt.Println("context canceled. [任务队列被关闭]")
		return b.ctx.Err()
...
func (b *PriorityQueue) Close() error {
	b.cancel()
	b.wg.Wait()
	return nil
}

func (b *PriorityQueue) AddWorker() error {
...
func (b *PriorityQueue) DoneWorker() {
	b.wg.Done()
	atomic.AddInt64(&b.addCount, -1)
}

// startDelayWorker 开启延迟队列任务
func (b *PriorityQueue) startDelayWorker() {
	b.wg.Add(1)
