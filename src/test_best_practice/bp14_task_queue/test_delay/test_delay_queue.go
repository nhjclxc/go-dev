...
var delayQueue *delay2.DelayQueue

func init() {
	ctx := context.Background()
	delayQueue = delay2.NewDelayQueue(ctx, 3)
}
...
func main() {
	...
	// http://127.0.0.1:8080/api/sendTask?id=1&name=task1&bizType=test1&delay=3
	// http://127.0.0.1:8080/api/sendTask?id=1&name=task1&bizType=test1&delay=10
	// http://127.0.0.1:8080/api/sendTask?id=3&name=task3&bizType=test3&delay=5
	e.GET("/api/sendTask", func(c *gin.Context) {
		...
		fmt.Printf("\n\n\n")
		c.JSON(200, gin.H{
			"message": "success: " + req.Name + time.Now().String() + ", len(delay) = " + fmt.Sprintf("%d", delayQueue.LenDelay()),
		})
