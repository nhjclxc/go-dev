...
func main() {
	...
	for ix := range values {
		go func() {
			...
			// 由于这里启用了协程，导致ix的值在协程开启到协程中使用这个值的这段时间内会被修改
			// 从而得到了错误的值
			// 要避免这个情况的发送，可以通过版本C解决（开启一个协程的时候显示的将数据直接传入每一个协程）
			fmt.Print(ix, " ")
		}()
	}
	fmt.Println()
	...
	/*
		...
		如果我们阻塞足够多的时间，让所有协程执行完毕，版本 B 的输出是：4 4 4 4 4。为什么会这样？
		在版本 B 的循环中，ix 变量实际是一个单变量，表示每个数组元素的索引值。
		因为这些闭包都只绑定到一个变量，这是一个比较好的方式，当你运行这段代码时，你将看见每次循环都打印最后一个索引值 4，而不是每个元素的索引值。
		因为协程可能在循环结束后还没有开始执行，而此时 ix 值是 4。

			版本 C 的循环写法才是正确的：调用每个闭包时将 ix 作为参数传递给闭包。
