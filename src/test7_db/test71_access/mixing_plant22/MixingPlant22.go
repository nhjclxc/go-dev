...
func main() {
	...
	dsn := fmt.Sprintf("Driver={Microsoft Access Driver (*.mdb)};Dbq=%s;PWD=%s;", *path, *pwd)
...
func queryProduce(db *sql.DB, now time.Time, oneMinuteAgo time.Time) []Produce {
	...
	for rows.Next() {
		var p Produce
		err = rows.Scan(
			&p.ID, &p.Code, &p.DatTim, &p.Attribute, &p.Contract, &p.Customer,
			...
			&p.Stamp, &p.Task, &p.Contacts, &p.ContTel, &p.Bend,
		)
...
func queryDosage(db *sql.DB, now time.Time, oneMinuteAgo time.Time) []Dosage {
	...
	for rows.Next() {
		var dosage Dosage
		err = rows.Scan(
			&dosage.ID, &dosage.Piece, &dosage.StorlD, &dosage.Storage, &dosage.MaterlD, &dosage.Material, &dosage.RecAmnt,
			&dosage.PlanAmnt, &dosage.FactAjnnt, &dosage.Fall, &dosage.FinTim, &dosage.Data, &dosage.Flag, &dosage.Stamp,
			&dosage.Rate, &dosage.CRC, &dosage.Mask,
		)
