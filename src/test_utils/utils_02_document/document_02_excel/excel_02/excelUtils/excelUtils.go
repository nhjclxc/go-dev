...
// 导入文件里面的数据
func ImportExcelFile[T any](filename string, firstColAxis int32, firstRowAxis int, headerKeys []string) []*T {
	// 1. 打开 Excel 文件
	...
	// ✅ 原则：类型参数 T 是局部的，只在当前函数内有效
	//Go 中，每个函数的 [T any] 都是自己独立定义的类型变量，即使名字相同，编译器也认为它们是不同的类型：

	// 显式传递泛型参数 是最佳实践：
	return doImportExcel[T](f, firstColAxis, firstRowAxis, headerKeys)
}

// 导入接口里面的数据 multipart.File
func ImportExcelByte[T any](file multipart.File, firstColAxis int32, firstRowAxis int, headerKeys []string) []*T {
...
// 执行导入
func doImportExcel[T any](f *excelize.File, firstColAxis int32, firstRowAxis int, headerKeys []string) []*T {
