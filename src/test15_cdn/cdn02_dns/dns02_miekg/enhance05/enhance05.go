...
func main() {
	...
	dnsServer.Handler = &enhance05DNSHandler{
		...
		// size=128是缓存大小
		// onEvict是内存被淘汰后的回调函数
		// ttl=time.Second*32，表示所有缓存的过期时间是32秒
		cache: expirable.NewLRU[string, *dns.Msg](128, nil, time.Second*32),
	}
...
type enhance05DNSHandler struct {
	// key = qname + qtype
	cache *expirable.LRU[string, *dns.Msg]
}
...
func (h enhance05DNSHandler) ServeDNS(w dns.ResponseWriter, r *dns.Msg) {
	...
	for _, question := range r.Question {
		...
		//    优先访问缓存的内容
		// =========
		cacheMsg, chaheFound := h.cache.Get(h.genCahceKey(qname, qtype))
		if chaheFound {
			// 找到记录了，直接用缓存里面的
			//cacheMsg,_ := value.(*dns.Msg)
			cacheMsg.Id = msg.Id
			msg.Answer = append(msg.Answer, cacheMsg.Answer...)

			fmt.Printf("使用缓存 dns question name：%s, qtype: %d \n", qname, qtype)

			// 继续处理下一个查询
			continue
		}

		fmt.Printf("未使用缓存 dns question name：%s, qtype: %d \n", qname, qtype)
		...
		if found {
			...
			h.cache.Add(h.genCahceKey(qname, qtype), msg.Copy())
		} else {
			// 使用系统的，进行上游递归查询
			...
			if upstreamResp != nil {
				...
				// 查询到的msg加入缓存
				h.cache.Add(h.genCahceKey(qname, qtype), upstreamResp.Copy())
			}
		}
	}
